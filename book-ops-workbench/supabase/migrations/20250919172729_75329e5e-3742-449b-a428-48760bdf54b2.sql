-- Update the account calculated values function to fix ATR calculation
-- ATR should only include opportunities that have a renewal_event_date
...
BEGIN
  ...
  FROM (
    ...
    SELECT 
      ...
      -- ATR: Only sum available_to_renew for opportunities with renewal_event_date
      SUM(CASE WHEN o.renewal_event_date IS NOT NULL THEN COALESCE(o.available_to_renew, 0) ELSE 0 END) as total_atr,
